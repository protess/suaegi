...
pub struct HookEvent {
    ...
    pub background_tasks_empty: Option<bool>,   // Stop에만. StopFailure는 갖지 않는다
}
...
- [ ] **이벤트 → 상태 매핑** (실측):
  - `UserPromptSubmit` → working. **`<task-notification>` 필터는 이 플랜에서 넣지 않는다.**
    ...
    `working`이 맞다. (2) 필터의 원래 동기는 "사람이 치지도 않은 프롬프트로 보인다"였는데
    **이 플랜은 프롬프트 텍스트를 표시하지 않는다.** 툴팁에 프롬프트를 띄우게 되면 그때 넣는다.

    (접두사는 이후 실측됐다 — `<task-notification>\n`으로 시작한다. 처음에 이 결정의 근거
    중 하나로 "미실측"을 들었으나 그건 이제 유효하지 않다. **위 두 이유만으로 결정이 유지된다.**
    실측 내용은 조사 §1.6에 있고, 페이로드의 `<note>`가 **같은 task-id가 여러 번 알릴 수 있다**고
    명시하므로 나중에 붙이더라도 **task-id로 중복 제거하면 안 된다.**)

    → **`HookEvent`에서 `prompt_is_task_notification` 필드를 뺀다.** 파싱만 하고 쓰지 않는
    필드는 썩고, 누군가 "이미 있으니" 쓰기 시작한다. 필요해지면 그때 되살린다.
