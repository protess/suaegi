...
1. **사용자의 Claude 설정을 건드리지 않는다.** 주입은 `--settings` 인라인 JSON으로만 한다
   (실측: 사용자 훅과 **병합**된다). Orca는 `~/.claude/settings.json`을 직접 고치는데 따라 하지 않는다.
...
```
**`background_tasks_empty == None`은 "비지 않음"으로 취급한다**(보수적). `Stop`에서는
실측으로 항상 존재하지만 `StopFailure` 페이로드는 미캡처다 — 없다고 `done`을 찍으면
백그라운드가 도는 중에 끝난 것으로 보인다. 반대 실수는 배지가 조금 늦게 도는 것뿐이다.
...
//   SUAEGI_HOOK_TOKEN  = token        ← env로만. --settings는 argv라 ps에 보인다
```
...
- [ ] **이벤트 → 상태 매핑** (실측):
  - `UserPromptSubmit` — 단, `prompt_is_task_notification`이면 **무시**(서브에이전트 완료가
    주입하는 합성 프롬프트다) → working
  - `PreToolUse`/`PostToolUse`/`PostToolUseFailure` → working
  - `PermissionRequest` → **waiting**. `AskUserQuestion` 특수 처리 없음(실측: 자동 허용이 아니다)
  - `Stop`/`StopFailure` + `background_tasks_empty == Some(true)` → done. 그 외 → **working 유지**
...
- [ ] **신뢰 대화상자**: 조사가 감지 수단을 확인하지 못했다. **이 플랜에서는 감지를 요구하지
  않는다** — 대신 `SessionStart` 이후 일정 시간 다른 훅이 하나도 안 오면 배지가 `Unknown`으로
  남고, 그것이 곧 사용자에게 보이는 신호다(0.3의 "훅 없음 → Unknown"이 이미 처리한다).
  **감지·사전 신뢰 심기는 follow-up으로 기록한다.**
