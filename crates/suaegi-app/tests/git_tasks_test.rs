...
async fn probe_keeps_the_detected_head_branch() {
    ...
    assert_eq!(
        head.as_deref(),
        Some("main"),
        "the head branch must not be dropped"
    );
...
async fn create_then_list_contains_exactly_the_new_worktree() {
    ...
    assert_eq!(
        matched.len(),
        1,
        "exactly one entry must be the worktree we created"
    );
}
...
async fn remove_takes_the_worktree_out_of_the_listing() {
    ...
    assert!(
        !list
            .iter()
            .any(|e| e.branch.as_deref() == Some(created.branch.as_str())),
