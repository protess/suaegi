...
/// 복원이 이만큼 지나도 안 끝나면 남은 잎을 실패로 확정한다.
///
/// **결과가 영영 안 오는 잎이 있을 수 있다**: 세션 스폰 워커가 패닉하면 그
/// 스레드가 죽고 `SessionStarted`가 발행되지 않는다. 그러면 하이드레이션
/// 게이트가 프로세스 수명 내내 닫혀 있고 **사용자의 어떤 조작도 저장되지
/// 않는다** — 아무 경고 없이. 느린 기계의 정상 스폰보다는 넉넉하고, 사용자가
/// 저장 없이 작업하는 시간보다는 짧아야 한다.
pub const RESTORE_WATCHDOG: Duration = Duration::from_secs(30);
