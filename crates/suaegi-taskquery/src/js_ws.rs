//! JS-faithful whitespace predicate + trim (plan Codex decision C1).
//!
//! Orca's `task-query.ts` leans on the ECMAScript whitespace set in EIGHT
//! places: four `\s` regexes and four `.trim()` calls. Neither Rust
...
//! (identical to `suaegi-search::js_trim` and `suaegi-automation::cron`) and use
//! it for every `\s`/`.trim()` site. Reverting either function to the Rust
//! built-ins is observable — see the C1 pin in `query.rs` tests.
...
/// set (see [`is_js_whitespace`]), NOT Rust's `str::trim`. Used for the four
/// `.trim()` sites in `task-query.ts` (`:73`, `:105`, `:161`, `:289`).
